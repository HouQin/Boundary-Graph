...
def train_model(
        ...
        learning_rate: float, reg_lambda: float, reg_gamma: float,
    ...
    reg_gamma = torch.tensor(reg_gamma, device=device)
        ...
        for phase in epoch_stats.keys():
            ...
            for idx, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    alpha_reg = torch.norm(model.propagation.linear1.weight)
                    # alpha_reg = torch.sum(torch.abs(model.propagation.linear1.weight))
                    beta_reg = torch.norm(model.propagation.linear2.weight)
                    loss = cross_entropy_mean + reg_lambda / 2 * l2_reg + reg_gamma / 2 * alpha_reg + reg_gamma / 2 * beta_reg
    ...
    result['log_pred'] = get_log_pred(model, attr_mat_norm, torch.arange(len(labels_all)))
...
def get_log_pred(model, attr_matrix, idx, batch_size=None):
    preds = []
    with torch.set_grad_enabled(False):
        log_preds = model._transform_features(attr_matrix)
